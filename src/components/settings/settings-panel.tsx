...
import { IconAdjustmentsHorizontal, IconArrowLeft, IconBell, IconLanguage, IconMailPlus, IconPalette, IconRefresh, IconTrash, IconUser } from '@tabler/icons-react'
...
function isLanguage(value: string): value is 'en' | 'tr' {
...
export function SettingsPanel({ settings, onChange, accounts, providerLogos, defaultAccountId, onSetDefault, onRemoveAccount, onStartAuth, onError, onBackToMail, isAddAccountOpen, onAddAccountOpenChange }: SettingsPanelProps) {
  ...
  const providerTabButtonRef = useRef<HTMLButtonElement>(null)

  const handleAddAccountOpenChange = (open: boolean) => {
    if (open) setProviderTab('gmail')
    onAddAccountOpenChange(open)
  }
  ...
  return <div className="settings-layout">
    ...
    <div className="settings-content">
      <header className="settings-heading">
        <div className="settings-heading-copy"><h1 id="settings-title">{tabs.find((tab) => tab.id === activeTab)?.label}</h1><span>{tabs.find((tab) => tab.id === activeTab)?.description}</span></div>
        {activeTab === 'accounts' ? <div className="accounts-heading-actions"><Button ref={addAccountButtonRef} className="account-add-button" onClick={() => handleAddAccountOpenChange(true)} aria-haspopup="dialog"><IconMailPlus aria-hidden="true" size={16} stroke={1.8} />{t('addAccount')}</Button></div> : null}
      </header>
            ...
            <article className="account-settings-row" key={account.id}>
              ...
              <div className="account-settings-meta">
                ...
                <div className="account-settings-actions">
                  ...
                  <Button className="account-settings-action" variant="ghost" size="icon" disabled={reconnectingAccountId !== null} aria-label={t('reconnectAccount')} title={t('reconnectAccount')} onClick={() => { void reconnectAccount(account) }}><IconRefresh aria-hidden="true" size={16} stroke={1.8} /></Button>
    ...
    <Dialog open={isAddAccountOpen} title={t('addAccount')} closeLabel={t('closeDialog')} returnFocusRef={addAccountButtonRef} initialFocusRef={providerTabButtonRef} onClose={() => handleAddAccountOpenChange(false)}>
      <div className="provider-tabs" role="tablist" aria-label={t('accountProviderTabs')}>
        {(['gmail', 'outlook'] as const).map((provider) => (
            <button ref={provider === 'gmail' ? providerTabButtonRef : undefined} className={`provider-tab ${providerTab === provider ? 'active' : ''}`} key={provider} id={`provider-tab-${provider}`} type="button" role="tab" aria-selected={providerTab === provider} aria-controls="provider-panel" tabIndex={providerTab === provider ? 0 : -1} data-tab-index={provider === 'gmail' ? 0 : 1} onKeyDown={(event) => moveTabFocus(event, provider === 'gmail' ? 0 : 1, 2, '.provider-tab', (nextIndex) => setProviderTab(nextIndex === 0 ? 'gmail' : 'outlook'))} onClick={() => setProviderTab(provider)} disabled={isAddingAccount}>
      ...
      <div className="provider-tab-panel" id="provider-panel" role="tabpanel" aria-labelledby={`provider-tab-${providerTab}`} aria-label={t(providerTab)}>
        <Button type="button" disabled={isAddingAccount} onClick={() => { setIsAddingAccount(true); void onStartAuth(providerTab).then(() => handleAddAccountOpenChange(false)).catch((error: unknown) => onError(error)).finally(() => setIsAddingAccount(false)) }}>{isAddingAccount ? t('connecting') : t(providerTab === 'gmail' ? 'continueWithGmail' : 'continueWithOutlook')}</Button>
      </div>
    </Dialog>
