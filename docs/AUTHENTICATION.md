...
5. In OpenMail, open Settings → OAuth credentials and enter the client
identifier and client secret. Do not use a `Web application` client for
OpenMail.

Desktop applications still use PKCE. The secret is stored in Windows
Credential Manager, never committed, logged, or embedded in the executable.
OAuth refresh tokens use the same Windows-protected store.
...
Open Settings → OAuth credentials and enter the public client ID. The value is
an application identifier, not a secret. Register `http://localhost` as a
mobile and desktop redirect URI in the Entra app registration. The running
adapter binds an available loopback port and uses the corresponding callback
path.
